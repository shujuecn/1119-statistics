...
```{R}
data |>
  pivot_longer(cols = c(WE, WD, W6),
               names_to = "Variable",
               values_to = "Value") |>
  ggplot(aes(x = factor(TREAT), y = Value)) +
  geom_boxplot() +
  facet_wrap( ~ Variable, scales = "free") +
  labs(title = "Comparison of Work Ability Across Groups (WE, WD, W6)", x = "Treatment Group", y = "Value") +
  plot_theme
```
...
data |>
  ...
  geom_point() +
  facet_wrap( ~ TREAT, labeller = as_labeller(c(`0` = "Standard Group", `1` = "Intensive Group"))) +
  labs(
...
# 创建整体散点图
plot_overall <- plot_scatter(data, group_title = "Overall Scatterplot with Linear Fit", color = "black")
